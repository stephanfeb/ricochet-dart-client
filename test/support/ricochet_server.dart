...
class RicochetTestServer {
  ...
  static String get adminUrl => Platform.environment['RICOCHET_TEST_PG'] ?? 'postgresql://monocelo:monocelo@localhost:5433/postgres';
  ...
  static Future<String?> available() async {
    ...
    if (psql.exitCode != 0) return 'no PostgreSQL at $adminUrl (${psql.stderr.toString().trim()}); start ../localnet or set RICOCHET_TEST_PG';
    return null;
  }
  ...
  static Future<RicochetTestServer?> start() async {
    ...
    final rng = Random();
    final db = 'ricochet_client_${rng.nextInt(1 << 30).toRadixString(16)}';
    ...
    final s = RicochetTestServer._(db, 40000 + rng.nextInt(20000), Directory.systemTemp.createTempSync('ricochet-client'));
    await s.launch();
    return s;
  }
